...
class ClusteringRequest(BaseModel):
    ...
    partial_fit: Optional[bool] = False
...
class ClusteringService:
    ...
    def perform_clustering(self, embeddings: np.ndarray, num_clusters: int, 
                          algorithm: str = "minibatch_kmeans", batch_size: int = 1000,
                          partial_fit: bool = False, cluster_centers: np.ndarray = None) -> tuple:
        """Perform clustering on embeddings using MiniBatchKMeans"""
        
        # Always use MiniBatchKMeans for scalability
        if partial_fit and self.clusterer is not None:
            ...
            # Initialize with existing cluster centers if provided
            if cluster_centers is not None:
                clusterer.cluster_centers_ = cluster_centers
                clusterer.n_features_in_ = len(cluster_centers[0])
                clusterer.partial_fit(embeddings)
            else:
    ...
    async def update_elasticsearch_clusters(self, document_embeddings: List[EmbeddingData], cluster_labels: np.ndarray):
        """Update Elasticsearch with cluster assignments"""
        try:
            for i, embedding_data in enumerate(document_embeddings):
                cluster_id = f"cluster_{int(cluster_labels[i])}"
                
                # Update document embedding with cluster ID
                doc_update = {
                    "doc": {
            ...
            logger.info(f"Updated {len(document_embeddings)} documents with cluster assignments")
...
async def cluster_documents(request: ClusteringRequest):
    ...
    try:
        ...
        cluster_labels, silhouette_avg, clusterer = clustering_service.perform_clustering(
            embedding_matrix, num_clusters, request.algorithm, 
            request.batch_size, request.partial_fit, cluster_centers
        )
            ...
            cluster_docs = [
                {
                    ...
                    "text_preview": request.embeddings[i].text_content[:200] + "..." if len(request.embeddings[i].text_content) > 200 else request.embeddings[i].text_content
                }
        ...
        # Update Elasticsearch with cluster assignments
        await clustering_service.update_elasticsearch_clusters(request.embeddings, cluster_labels)
    ...
    try:
        ...
        for hit in response['hits']['hits']:
            documents.append({
                ...
                "text_preview": hit['_source']['textContent'][:200] + "..." if len(hit['_source']['textContent']) > 200 else hit['_source']['textContent']
            })
